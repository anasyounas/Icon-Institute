/**
 * Migrate regional projects from hp.icon-institute.de into the CMS API.
 *
 * Usage:
 *   CMS_PASSWORD=... REGION=asia WP_PATH=projects-asia node scripts/migrate-region-projects.mjs
 *   CMS_PASSWORD=... REGION=central-america-caribbean WP_PATH=projects-central_america_caribbean node scripts/migrate-region-projects.mjs
 *   CMS_PASSWORD=... REGION=south-america WP_PATH=projects-south_america node scripts/migrate-region-projects.mjs
...
const WP_PATH = (process.env.WP_PATH || '').replace(/^\/+|\/+$/g, '');
...
if (!REGION || !WP_PATH) {
  console.error('Set REGION (cms slug) and WP_PATH (e.g. projects-south_america)');
  process.exit(1);
}

const OUT_DIR = join(__dirname, '_extracted', `${REGION}-migration`);
...
async function collectRegionIds() {
  const ids = [];
  for (let p = 1; p <= 30; p++) {
    const url =
      p === 1 ? `${WP}/${WP_PATH}/` : `${WP}/${WP_PATH}/page/${p}/`;
    ...
    if (!html.includes(`/${WP_PATH}/page/${p + 1}/`)) break;
    await sleep(200);
  }
  return [...new Set(ids)];
}
...
function existingKey(title, region) {
  return `${region}::${normTitle(title)}`;
}

async function upsertProject(token, existingByKey, payload) {
  const key = existingKey(payload.title, payload.region);
...
async function main() {
  console.log(`API: ${API}`);
  console.log(`REGION=${REGION} WP_PATH=${WP_PATH}`);
  ...
  for (const p of existing) existingByKey.set(existingKey(p.title, p.region), p);
  console.log(`CMS projects loaded: ${existing.length}`);

  const ids = await collectRegionIds();
  console.log(`${REGION} WP ids: ${ids.length}`);
  ...
  for (let i = 0; i < slice.length; i++) {
    ...
    try {
      ...
      if (!scraped.title || scraped.title.trim().length < 3) {
        throw new Error('empty title on source page — skipped');
      }
      ...
      const result = await upsertProject(token, existingByKey, payload);
      if (result.item) {
        existingByKey.set(existingKey(result.item.title, result.item.region), result.item);
      } else {
        existingByKey.set(existingKey(payload.title, payload.region), {
          id: result.id,
          title: payload.title,
